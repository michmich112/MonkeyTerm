...
// Attempts to create a new managed Term
...
// Makes the term cursor invisible
...
// Get the initial X position of the canvas/cursor
func (t *Term) InitialX() int {
	return t.initialPosition[0]
}

// Get the initial Y position of the canvas/cursor
...
// Appends an ANSI Goto position code to the output buffer
// For transactional use only
// !! unsafe
func (t *Term) TxGotoPosition(x, y int) {
...
// Goes to the desired relative position on the terminal relative to the starting point
// For transactional use only
// !! unsafe
func (t *Term) TxGotoRelativePosition(x, y int) {
	t.TxGotoPosition(x+t.InitialX(), y+t.InitalY())
}
...
// Outputs bytes to the output buffer to be printed
// For transactional use only
// !! unsafe
func (t *Term) TxOutputBytes(bytes []byte) {
	t.outputBytes(bytes)
}
...
// Writes out the current contents of the output buffer
// For transactional use only
// !! unsafe
func (t *Term) TxWriteOutBuf() error {
	return t.writeOutBuf()
}
...
// Saves the current cursor position (DEC)
...
func (t *TermTx) Execute() {
	...
	t.t.TxWriteOutBuf() //write out all changes
	t.t.L.Unlock()
}
