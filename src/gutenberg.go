...
func (g *Gutenberg) End() {
	g.Term.GotoRelativePosition(0,0) // go to canvas origin
...
func (s *Section) Reprint(text []byte) error {
	...
	tui.NewTermTx(s.gutenberg.Term).
		AddAction(func(t *tui.Term) { t.TxGotoRelativePosition(s.Start, 0) }).
		AddAction(func(t *tui.Term) { t.TxOutputBytes([]byte(out)) }).
		Execute()
